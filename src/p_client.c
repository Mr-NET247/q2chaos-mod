...
void ClientObituary (edict_t *self, edict_t *inflictor, edict_t *attacker)
{
	...
	if (deathmatch->value)
	{
		...
		if (message)
		{
			...
			{
				i = (int) (random() * (float) chat_linecount[BOT_CHAT_SELFKILL]);

				Bot_Say (attacker, 0, "%s: ", self->client->pers.netname);
				Bot_Say (attacker, 0, chat_text[BOT_CHAT_SELFKILL][i], self->client->pers.netname);
				Bot_Say (attacker, 0, "\n");
			}
			return;
		}
		...
		if (attacker && attacker->client)
		{
			...
			if ((strcmp(attacker->classname, "bot") == 0) && visible (self, attacker))
			{
				...
				if (!TeamMembers(self, attacker))
				{
					...
					i = (int) (random() * (float) chat_linecount[BOT_CHAT_KILL]);

					Bot_Say (attacker, 0, "%s: ", attacker->client->pers.netname);
					Bot_Say (attacker, 0, chat_text[BOT_CHAT_KILL][i], self->client->pers.netname);
					Bot_Say (attacker, 0, "\n");
				}
				else
				{
					i = (int) (random() * (float) chat_linecount[BOT_CHAT_TEAMKILL]);

					Bot_Say (attacker, 0, "%s: ", attacker->client->pers.netname);
					Bot_Say (attacker, 0, chat_text[BOT_CHAT_TEAMKILL][i], self->client->pers.netname);
					Bot_Say (attacker, 0, "\n");
				}
			}
			return;
...
void ClientBeginDeathmatch (edict_t *ent)
{
	...
	bprintf2 (PRINT_HIGH, "%s entered the matrix\n", ent->client->pers.netname);
...
//ZOID
	...
	if (game.maxclients > 1){
		gi.dprintf ("Call trans opt: received\n");
		gi.dprintf ("Trace program: running.\n");
		gi.dprintf ("Trace complete. Call origin: %s\n", ent->client->pers.netname);
...
void ClientDisconnect (edict_t *ent)
{
	...
	bprintf2 (PRINT_HIGH, "%s cut their hard line\n", ent->client->pers.netname);
